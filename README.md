...
3.  **(Optional) Jinja2 Templates Source Directory**: If you plan to use custom configuration templates for services:
    * Create a directory named `jinja2-templates/` in the **same directory where you will run the `ansible-setup-wizard.sh` script.**
...
2.  **Make the Script Executable**:
    ```bash
    chmod +x ansible-setup-wizard.sh
    ```
...
3.  **(Optional) Prepare Jinja2 Templates Source**:
    ...
    ```
    .
    ├── ansible-setup-wizard.sh  <-- Your script
